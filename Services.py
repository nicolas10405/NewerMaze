...
def status_update(number, dubs, route_length):
    ...
    print("Step 2 of 2 (Shortening path...): [" + progress_string + "] ",
          str(number) + " deviations eliminated | current path length: " + str(route_length) + " steps")

    return time.time()


def clean_up_route(long_route, hide_stats):
    ...
    while continue_to_check:
        ...
        for i in range(len(long_route_rev)):
            if long_route_rev[i] not in check_list:
                ...
                if time.time() - last_execution_time > 0.5 and not hide_stats:
                    last_execution_time = status_update(status_counter, number_of_duplicates_assumed, len(long_route))
...
def save_s_matrix(filename, matrix, hide_stats):
    f = open(str(filename) + ".smx", "wt")
    f.write(str(matrix))
    f.close()
    if not hide_stats: print("***Matrix saved to file: " + filename)


def open_s_matrix(filename, matrix):
    if os.path.exists(str(filename) + ".smx"):
        f = open(str(filename) + ".smx", "rt")
        print("***Matrix opened from file: " + filename + "\nConverting and reading to internal list... (this may take a while)\n")
        matrix = f.read()
